...
namespace ClassManagementAPI.Controllers
{
    ...
    public class ClassController : ControllerBase
    {
        ...
        [HttpPut("{classId}/user")]
        public async Task<IActionResult> UpdateUserInClass(string classId, [FromBody] UpdateUserDTO updateUserDTO)
        {
            try
            {
                var user = await _userRepository.GetUserDTOByIdAsync(updateUserDTO.UserId);
                if (user == null)
                    return NotFound("User not found");

                if (user.RoleId != "admin")
                {
                    updateUserDTO.RoleId = "admin";
