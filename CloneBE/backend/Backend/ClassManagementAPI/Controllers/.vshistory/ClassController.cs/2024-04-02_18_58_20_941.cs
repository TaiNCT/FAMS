...
namespace ClassManagementAPI.Controllers
{
    ...
    public class ClassController : ControllerBase
    {
        ...
        public ClassController(ILogger<ClassController> logger, IElasticClient client, IMapper mapper, IClassRepository classRepository, ILocationRepository locationRepository, IUserRepository userRepository, IClassUserRepository classUserRepository, ISyllabusRepository syllabusRepository, FamsContext FM)
        ...
        [HttpDelete("{classId}/DeleteUser")]
        public async Task<IActionResult> DeleteUserFromClass(string classId, [FromQuery] string userId)
        {
            try
            {
                await _userRepository.DeleteUserFromClassAsync(userId, classId);
        ...
        [HttpPut("{classId}/fsu")]
        public IActionResult UpdateFsuInClass(string classId, [FromBody] UpdateFsuInClassDto updateDto)
        {
            try
            {
                ...
                var fsuEntity = _context.Fsus.FirstOrDefault(f => f.FsuId == updateDto.FsuId);
                if (fsuEntity == null)
                {
                    return NotFound();
                }

                // Update FSU properties
                fsuEntity.Name = updateDto.Name;
                fsuEntity.Email = updateDto.Email;

                // Update FSU reference in class
                classEntity.FsuId = fsuEntity.FsuId;
        ...
        [HttpPut("{classId}/trainingprogram")]
        public IActionResult UpdateTrainingProgramInClass(string classId, [FromBody] UpdatedTrainingProgramInClassDto updateDto)
        {
            try
            {
                var classEntity = _context.Classes.FirstOrDefault(c => c.ClassId == classId);
                ...
                var trainingProgramEntity = _context.TrainingPrograms.FirstOrDefault(tp => tp.TrainingProgramCode == updateDto.TrainingProgramCode);
                if (trainingProgramEntity == null)
                {
                    return NotFound();
                }

                // Update Training Program properties
                trainingProgramEntity.TrainingProgramCode = updateDto.TrainingProgramCode;
                trainingProgramEntity.UpdatedBy = updateDto.UpdatedBy;
                trainingProgramEntity.UpdatedDate = updateDto.UpdatedDate;
                trainingProgramEntity.Days = updateDto.Days;
                trainingProgramEntity.Hours = updateDto.Hours;
                trainingProgramEntity.Name = updateDto.Name;
        ...
        [HttpPut("{classId}/attendeetype")]
        public IActionResult UpdateAttendeeTypeInClass(string classId, [FromBody] UpdatedAttendeeInClassDto updateDto)
        {
            try
            {
                var classEntity = _context.Classes.FirstOrDefault(c => c.ClassId == classId);
                ...
                var attendeeTypeEntity = _context.AttendeeTypes.FirstOrDefault(at => at.AttendeeTypeId == updateDto.AttendeeTypeId);
                if (attendeeTypeEntity == null)
                {
                    return NotFound(); // Attendee Type not found
                }

                // Update Attendee Type properties
                classEntity.AttendeeLevelId = attendeeTypeEntity.AttendeeTypeId;
