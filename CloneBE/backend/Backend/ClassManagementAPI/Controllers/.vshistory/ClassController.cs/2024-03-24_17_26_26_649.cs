...
namespace ClassManagementAPI.Controllers
{
    ...
    public class ClassController : ControllerBase
    {
        ...
        [HttpPut("assign-admin/{classId}")]
        public async Task<IActionResult> AssignAdminByClassId(string classId)
        {
            try
            {
                var role = await _classRepository.GetRoleByIdAsync("adminRoleId");
                // Check if role with adminRoleId exists
                if (role == null)
                {
