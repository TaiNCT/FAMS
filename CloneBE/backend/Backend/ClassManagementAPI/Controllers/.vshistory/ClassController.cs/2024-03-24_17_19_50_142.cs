...
namespace ClassManagementAPI.Controllers
{
    ...
    public class ClassController : ControllerBase
    {
        ...
        [HttpPut("promote-to-admin/{classId}")]
        public async Task<IActionResult> PromoteUserToAdmin(string classId)
        {
            try
            {
                // Get the user associated with the provided classId
                var user = await _userRepository.GetUsersById(classId)
                    .Include(u => u.Role)
                    .FirstOrDefaultAsync();

                if (user == null)
                {
                    return NotFound("User not found for the provided classId");
                }
                ...
                // If the user doesn't have a role, create a new one and assign "admin" role
                if (user.Role == null)
                {
                ...
                {
                    // If the user has a role, update it to "admin"
                ...
                return Ok("User role updated to admin successfully");
            }
            catch (Exception ex)
            {
                return StatusCode(500, "Internal server error");
