...
namespace ClassManagementAPI.Controllers
{
    ...
    public class ClassController : ControllerBase
    {
        ...
        public async Task<IActionResult> RemoveUserFromClass([FromQuery] string userId)
        {
            try
            {
                await _userRepository.RemoveUserFromClassAsync(userId);
                return Ok("User removed from class successfully.");
            }
            catch (Exception ex)
            {
