...
import fileURL from "../../../../assets/template/ImportUpdateUserPermissionTempate.xlsx";

const handleDownload = async () => {
  const link = document.createElement("a");
  link.target = "_blank";
  link.download = "ImportUpdateUserPermissionTempate.xlsx";
...
const ImportPermissionPopup = ({ onCancel, onUpdateData }) => {
  const [fileList, setFileList] = useState([]);
  const [errorMessage, setErrorMessage] = useState(null); // State để lưu trữ thông báo lỗi
  ...
  const handleImport = () => {
    ...
    axios
      .post(`${backend_api}/api/import-user-perms`, formData, {
        headers: {
      ...
      .then((response) => {
        if (response.data.isSuccess) {
          toast.success("Import new role permissions success", {
            position: "top-right",
            autoClose: 3000,
            hideProgressBar: false,
            closeOnClick: true,
          ...
          toast.error("Import new role permissions failed", {
            position: "top-right",
            autoClose: 3000,
            hideProgressBar: false,
            closeOnClick: true,
...
export default ImportPermissionPopup;
