...
import MultipleSelectChip from "./SelectLocaion";
...
import { fetchFilteredData } from "../api/FilterApi";
import { FsuListApi, TrainingProgramListAPI1, LocationList } from "../api/ListApi";
import BasicSelectTrainer from "./BasicSelectTrainer";

export default function FilterPopOver({ fetchData, anchorEl, setAnchorEl, onFilteredData, setIsFiltered })
{
  const open = Boolean(anchorEl);
  const id = open ? "filter-popover" : undefined;
  const labels = ["Planning", "Openning", "Closed"];
  ...
  const handleFilterSubmit = React.useCallback(async () =>
  {
    ...
    onFilteredData(data);
    handleClose();
  }, [checkedBoxStatus, selectedFSU, selectedProgram, checkedBoxAttendee, selectedLocationIds, checkedBoxTime, selectedStartDate, selectedEndDate]);
          ...
          <form action="">
            ...
            <section style={{ marginTop: "10px", display: "flex" }}>
              ...
              <div style={{ display: "flex", marginLeft: "120px" }}>
                <p className={StyleSCss.selectName} style={{ marginTop: "10px" }}>Training Program</p>
