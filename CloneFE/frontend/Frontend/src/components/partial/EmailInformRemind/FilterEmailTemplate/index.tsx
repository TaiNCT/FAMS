...
import { EmailTemplate, TableParams } from "../EmailTable/EmailConfigTable";
import style from "./style.module.scss";
// import axiosAuth from "../../EmailInformRemind/api/axiosAuth";
				...
				<Modal open={this.state.isModalOpen} onCancel={this.toggleModal} footer={null} closable={false} className={style.popup}>
					<Form onFinish={this.handleSubmit} className={style.modalBody}>
						...
						<Form.Item label={<span className={style.tag}>Categories</span>} className={style.form_group}>
							<Checkbox.Group onChange={(values) => this.setState({ type: values })} value={this.state.type || (this.props.searchParams.type as [])}>
								<Checkbox name="reserve" value="1">
									Reserve
								</Checkbox>
								<Checkbox name="remind" value="2">
									Remind
								</Checkbox>
								<Checkbox name="notice" value="3">
									Notice
								</Checkbox>
								<Checkbox name="other" value="4">
									Other
								</Checkbox>
							</Checkbox.Group>
						</Form.Item>
