...
import { useEmailContext } from "./AllCategoriesTable";
import { emailContext } from "@/pages/EmailConfigurationPage";
import { context } from "./AllCategoriesTable";
...
export const EmailTable: React.FC<EmailTableProps> = ({ newDataUpdate, tableParams, setTableParams, searchParams }) => {
	...
	const columns: TableProps<EmailTemplate>["columns"] = [
		{
			...
			render: (programName) => (
				<div>
					<Typography className="ant-table-cell-name"> {programName}</Typography>
				</div>
			),
			sortIcon: () => (
				<div>
		...
		},
		{
			...
			title: "Status",
			dataIndex: "status",
			render: (status) => <Typography>{status === 1 ? <Chip label="Active" color="success" /> : <Chip sx={{ color: "white", bgcolor: "#B9B9B9" }} label="Inactive" />}</Typography>,
		},
		{
			key: "description",
			title: "Description",
	...
	useEffect(() => {
		const fetchData = async () => {
			...
			setIsLoading(true);
			try {
				let url = `${backend_api}/api/emailTemplates/list?page=${tableParams.pagination?.current ? tableParams.pagination.current : 0}&pageSize=${tableParams.pagination?.pageSize}`;
				...
				if (searchParams.type) {
					...
					urlTotalPage += `&type=${searchParams.type}`;
				}
				...
				if (searchParams.name || searchParams.description) {
					data_afer_filter = data.filter((e) => e.description.includes(searchParams.description) || e.name.includes(searchParams.name)) as EmailTemplate[];
